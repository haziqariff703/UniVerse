...
- **Simplified Hover Interactions**: Made social buttons always visible (opacity-50) instead of "pop-in" animation, reducing visual noise while maintaining accessibility.
- **Improved Text Readability**: Enhanced bottom gradient from `from-[#0a0a0f]` to `from-black/95 via-black/40` for better text contrast against any photo background.
- **Performance Optimization**: Standardized all transitions to `duration-300` targeting 60fps performance.
- **Editorial Lift Animation**: Applied subtle `scale-[1.02] translate-y-[-5px]` on hover for premium floating effect.

### Fixed

- **Browser Cache Issue**: Resolved Vite cache conflicts that prevented deleted 3D card components from being properly removed from build.
...
- # **Added**: Created physics-based Lanyard component and reusable StatCard for the new Profile Dashboard. (2026-01-20)
  > > > > > > > bed15b58a95246acc74d0678a627004686a114f2
