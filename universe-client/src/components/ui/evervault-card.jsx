...
export const EvervaultCard = ({ text, className, children }) => {
  ...
  return (
    ...
    >
      {/* Matrix Background - Only visible on hover */}
      <div
        className={cn(
...
export function CardPattern({ mouseX, mouseY, randomString }) {
  ...
  return (
    <div className="pointer-events-none">
      ...
      <motion.div
        className="absolute inset-0 rounded-xl bg-gradient-to-r from-purple-500 to-cyan-500 opacity-0 group-hover/card:opacity-100 backdrop-blur-xl transition duration-500"
        style={style}
      />
      <motion.div
        ...
        style={style}
      >
        <p className="absolute inset-x-0 h-full break-words whitespace-pre-wrap text-white font-mono text-xs font-bold transition duration-500">
          {randomString}
        </p>
      </motion.div>
    </div>
