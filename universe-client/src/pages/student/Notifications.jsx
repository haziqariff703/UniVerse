  ...
  return (
    <div className="min-h-screen pt-28 pb-20 px-4 md:px-8 max-w-4xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4">
        <div>
          <h1 className="text-4xl md:text-5xl font-neuemontreal font-bold text-white mb-2 flex items-center gap-3 animate-in fade-in slide-in-from-bottom-4 duration-500">
            Notification{" "}
            <span className="text-sm font-sans font-normal px-2 py-1 bg-accent rounded-full text-white align-middle">
              {notifications.filter((n) => !n.isRead).length} New
            </span>
          </h1>
          <p className="text-starlight/60 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-100">
        ...
        <div className="flex gap-3 animate-in fade-in slide-in-from-right-4 duration-500 delay-200">
          ...
          <button
            ...
            className="px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-sm font-medium text-gray-300 transition-colors flex items-center gap-2"
          >
      ...
      <div className="space-y-4">
        {filteredNotifications.length > 0 ? (
          filteredNotifications.map((notif, idx) => (
            <div
              ...
              className={`relative p-5 rounded-2xl border transition-all duration-300 animate-in fade-in slide-in-from-bottom-2 ${
                notif.isRead
                  ...
                  : "bg-white/10 border-accent/30 shadow-lg shadow-accent/5"
              }`}
              style={{
            ...
            >
              <div className="flex items-start gap-4">
                <div
                  className={`mt-1 p-2 rounded-full bg-white/5 border border-white/10 shrink-0`}
                >
                  {getIcon(notif.type)}
                </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {!notif.isRead && (
                        <button
                          ...
                          className="ml-2 text-xs text-accent hover:text-white transition-colors underline"
                          title="Mark as read"
                        >
                          Dismiss
                        </button>
          ...
          <div className="text-center py-20 bg-white/5 rounded-2xl border border-white/5 border-dashed animate-in fade-in">
